    ...
    it(`Should pass if an edit with the smartCrop keyname is passed to
      ...
      await imageHandler
        ...
        .then((result) => {
          ...
          assert.deepEqual(
            originalImageData.options.input !== result.options.input,
            true
          );
        })
        .catch((err) => {
...
describe("getOverlayImage()", () => {
  describe("001/validParameters", () => {
    it(`Should pass if the proper bucket name and key are supplied,
      ...
      assert.deepEqual(
        result,
        Buffer.from(
          "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAACXBIWXMAAAsSAAALEgHS3X78AAAADUlEQVQI12P4z8CQCgAEZgFlTg0nBwAAAABJRU5ErkJggg==",
          "base64"
        )
      );
    });
  ...
  describe("002/imageDoesNotExist", async () => {
    it(`Should throw an error if an invalid bucket or key name is provided,
      ...
      // Assert
      imageHandler
        .getOverlayImage("invalidBucket", "invalidKey")
        .then((result) => {
...
describe("getBoundingBox()", () => {
  ...
  describe("002/errorHandling", () => {
    it("Should simulate an error condition returned by Rekognition", async () => {
      ...
      // Assert
      imageHandler
        .getBoundingBox(currentImage, faceIndex)
        .then((result) => {
          assert.equal(typeof result, Error);
        })
        .catch((err) => {
