...
class ImageRequest {
    ...
    try {
      ...
      if (this.outputFormat) {
        ...
        if (
          ...
          const qualityKey = Object.keys(this.edits).filter((key) =>
  ...
  parseImageKey(event, requestType) {
    ...
    if (requestType === "Thumbor" || requestType === "Custom") {
      return decodeURIComponent(
        event.path
          .replace(/\d+x\d+\/|filters[:-][^/;]+|\/fit-in\/+|^\/+/g, "")
          .replace(/^\/+/, "")
      );
    }
  ...
  parseRequestType(event) {
    ...
    const matchThumbor = new RegExp(
      /^(\/?)((fit-in)?|(filters:.+\(.?\))?|(unsafe)?).*(.+jpg|.+png|.+webp|.+tiff|.+jpeg)$/i
    );
    ...
    if (matchCustom.test(path) && definedEnvironmentVariables) {
      // use rewrite function then thumbor mappings
      return "Custom";
    }
    if (matchThumbor.test(path)) {
