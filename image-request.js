...
class ImageRequest {
    ...
    try {
      ...
      if (this.outputFormat) {
        ...
        if (requestType.includes(this.requestType) && acceptedValues.includes(this.outputFormat)) {
          const qualityKey = Object.keys(this.edits).filter((key) => acceptedValues.includes(key))[0];
  ...
  async getOriginalImage(bucket, key) {
    ...
    try {
      ...
      return Promise.reject({
        status: (err.code === 'NoSuchKey') ? 404 : 500,
        code: err.code,
        message: err.message,
      });
    }
  ...
  parseRequestType(event) {
    ...
    if (matchDefault.test(path)) { // use sharp
      return 'Default';
    } if (matchCustom.test(path) && definedEnvironmentVariables) { // use rewrite function then thumbor mappings
      return 'Custom';
    } if (matchThumbor.test(path)) { // use thumbor mappings
      return 'Thumbor';
    }
    throw {
      status: 400,
